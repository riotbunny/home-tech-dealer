  ...
  return (
    <section className="py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="mt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-sm space-y-6">
          <div className="flex items-center justify-between pb-3 border-b border-slate-800/50">
            <div>
              <h3 className="text-base font-bold text-white">Mover Bundle Savings Calculator</h3>
          ...
          <div className="space-y-3">
            {/* Core Internet */}
            <div className="p-4 rounded-2xl bg-indigo-900/30/60 border border-blue-200 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-blue-600 text-white flex items-center justify-center">
                  <Wifi className="w-5 h-5" />
                </div>
                <div>
                  <div className="text-sm font-bold text-white">High-Speed Fiber or Cable Internet</div>
                  <div className="text-xs text-slate-500">Primary home connection</div>
                </div>
              </div>
              <span className="text-xs font-bold text-indigo-700 bg-blue-100 px-2.5 py-1 rounded-lg">
                Included
              </span>
            </div>
            ...
            <label className={`p-4 rounded-2xl border flex items-center justify-between cursor-pointer transition-all ${
              hasMobile ? 'bg-emerald-900/30/40 border-emerald-300' : 'bg-slate-900 border-slate-800'
            }`}>
              <div className="flex items-center gap-3">
                <input
                  type="checkbox"
                ...
                <div>
                  <div className="text-sm font-bold text-white">Bundle Unlimited Mobile Line</div>
            ...
            {/* Live TV */}
            <label className={`p-4 rounded-2xl border flex items-center justify-between cursor-pointer transition-all ${
              hasStreaming ? 'bg-emerald-900/30/40 border-emerald-300' : 'bg-slate-900 border-slate-800'
            }`}>
              <div className="flex items-center gap-3">
                <input
                  type="checkbox"
                ...
                <div>
                  <div className="text-sm font-bold text-white">Add Live TV &amp; Sports Streaming</div>
            ...
            {/* Smart Home */}
            <label className={`p-4 rounded-2xl border flex items-center justify-between cursor-pointer transition-all ${
              hasSecurity ? 'bg-emerald-900/30/40 border-emerald-300' : 'bg-slate-900 border-slate-800'
            }`}>
              <div className="flex items-center gap-3">
                <input
                  type="checkbox"
                ...
                <div>
                  <div className="text-sm font-bold text-white">Smart Home Security &amp; Doorbell</div>
          ...
          <div className="p-5 rounded-2xl bg-emerald-900/30 border border-emerald-200 flex flex-wrap items-center justify-between gap-4">
            <div>
              <div className="text-xs text-slate-300 font-semibold">Your Estimated Mover Savings:</div>
              <div className="text-2xl sm:text-3xl font-extrabold text-emerald-700 mt-0.5">
                ${annualSavings}
                <span className="text-xs font-normal text-slate-300"> / year in bundle discounts</span>
              </div>
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-700 hover:to-blue-700 shadow-md shadow-indigo-500/20 text-white font-bold text-xs flex items-center gap-1.5 shadow-sm transition-all"
            >
        ...
        <div className="lg:col-span-5 bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-sm space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-slate-800/50">
            <h3 className="text-base font-bold text-white flex items-center gap-2">
              <Calendar className="w-4 h-4 text-indigo-600" />
              <span>Moving Day Tech Checklist</span>
            </h3>
            <span className="text-xs font-bold text-indigo-700 bg-indigo-900/30 px-2 py-0.5 rounded-full">
              Interactive
            </span>
          </div>
              ...
              return (
                <div
                  ...
                  className={`p-3 rounded-xl border cursor-pointer flex items-start gap-3 transition-all ${
                    done ? 'bg-emerald-900/30 border-emerald-200 text-slate-300' : 'bg-slate-950/50 border-slate-800 text-slate-200 hover:bg-slate-800'
                  }`}
                >
                  <div className={`w-4 h-4 rounded mt-0.5 flex items-center justify-center shrink-0 border ${
                    done ? 'bg-emerald-600 border-emerald-600 text-white' : 'border-slate-700 bg-slate-900'
                  }`}>
                    {done && <Check className="w-3 h-3 stroke-[3]" />}
                  </div>
                  <span className={`text-xs ${done ? 'line-through text-slate-500' : 'text-slate-100 font-medium'}`}>
                    {item.text}
                  </span>
                </div>
              );
          ...
          <div className="pt-4 border-t border-slate-800/50 flex items-center justify-between">
            <div>
              ...
              <div className="text-xs font-bold text-white">Toll-Free Mover Setup Desk</div>
            </div>
            <a
              href={telHref}
              className="px-3.5 py-2 rounded-xl bg-emerald-900/30 hover:bg-emerald-100 text-emerald-800 text-xs font-bold flex items-center gap-1.5 border border-emerald-200 transition-colors"
            >
