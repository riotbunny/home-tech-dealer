  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-fade-in">
      <div className="relative w-full max-w-5xl bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
        ...
        {/* Address Banner */}
        <div className="px-6 py-2.5 bg-indigo-900/30/60 border-b border-blue-100 flex items-center justify-between text-xs">
        ...
        <div className="p-6 overflow-y-auto flex-1">
          {cartPlans.length === 0 ? (
            <div className="py-16 text-center space-y-3">
              ...
              <h4 className="text-base font-bold text-slate-100">Your Comparison Cart is Empty</h4>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                {cartPlans.map((plan) => (
                  <div
                    key={plan.id}
                    className="rounded-2xl border border-slate-800 bg-slate-950/50/50 p-5 flex flex-col justify-between"
                  >
                    <div>
                      {/* Provider Header */}
                      <div className="flex items-center justify-between mb-3 pb-3 border-b border-slate-800">
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <span className="font-bold text-sm text-white">{plan.providerName}</span>
                        </div>
                        <button
                      ...
                      <div className="mt-4 space-y-2 text-xs text-slate-200">
                        ...
                        <div className="p-2.5 rounded-xl bg-slate-900 border border-slate-800 flex justify-between">
                          ...
                          <span className="font-bold text-white">{plan.uploadSpeed}</span>
                        </div>
                        <div className="p-2.5 rounded-xl bg-slate-900 border border-slate-800 flex justify-between">
                          <span className="text-slate-500">Equipment:</span>
                          <span className="font-medium text-slate-100">{plan.equipmentFee}</span>
                        </div>
                        <div className="p-2.5 rounded-xl bg-slate-900 border border-slate-800 flex justify-between">
                          <span className="text-slate-500">Setup Window:</span>
                          <span className="font-medium text-slate-100">{plan.installationSla}</span>
                        </div>
                      </div>
                ...
                {cartPlans.length < 3 && (
                  <div className="rounded-2xl border-2 border-dashed border-slate-800 p-6 flex flex-col items-center justify-center text-center text-slate-500 bg-slate-950/50/40">
                    <Layers className="w-8 h-8 mb-2 opacity-40 text-slate-500" />
                    <div className="text-xs font-bold text-slate-300">Slot {cartPlans.length + 1} Open</div>
                    <p className="text-xs text-slate-500 mt-1 max-w-[200px]">
                      Add {3 - cartPlans.length} more plan{3 - cartPlans.length > 1 ? 's' : ''} to compare side-by-side.
                    </p>
                    <button
                      onClick={onClose}
                      className="mt-3 px-3 py-1.5 rounded-xl bg-slate-900 border border-slate-700 text-slate-200 hover:bg-slate-800 text-xs font-semibold"
                    >
                      + Browse More Plans
                    </button>
                  </div>
        ...
        {cartPlans.length > 0 && (
          <div className="px-6 py-4 bg-slate-950/50 border-t border-slate-800 flex flex-wrap items-center justify-between gap-4">
            <div className="text-xs text-slate-200 flex items-center gap-2">
              ...
              <span>Call <strong className="text-white">{phoneNumber}</strong> with Promo Code <strong className="text-indigo-700 font-mono">PROMO-FREE-INSTALL</strong> for Free Setup</span>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-amber-900/300 hover:bg-amber-600 text-white font-bold text-xs transition-all shadow-xs"
              >
              ...
              <button
                onClick={onClose}
                className="px-4 py-2.5 rounded-xl bg-slate-900 border border-slate-700 hover:bg-slate-800 text-slate-200 text-xs font-semibold"
              >
                Close
              </button>
            </div>
          ...
          <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl p-6 relative">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800/50">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-base font-bold text-white">Email Comparison Quote</h4>
              </div>
              <button
            ...
            {emailSent ? (
              <div className="py-8 text-center space-y-2">
                ...
                <h5 className="text-base font-bold text-white">Comparison Quote Sent!</h5>
                <p className="text-xs text-slate-500 max-w-xs mx-auto">
                  A side-by-side summary with promotional codes has been sent to <strong className="text-white">{customerEmail}</strong>.
                </p>
                <div className="pt-3">
                  <span className="text-xs font-semibold text-emerald-700 bg-emerald-900/30 px-2.5 py-1 rounded-full border border-emerald-200">
              ...
              <form onSubmit={handleSendEmail} className="mt-4 space-y-4 text-xs">
                ...
                <div>
                  <label className="block text-slate-200 font-semibold mb-1">Your Full Name</label>
                  <input
                    type="text"
                    required
                    ...
                    className="w-full px-3.5 py-2.5 bg-slate-950/50 border border-slate-700 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:border-blue-600"
                  />
                </div>

                <div>
                  <label className="block text-slate-200 font-semibold mb-1">Your Email Address</label>
                  <input
                    type="email"
                    required
                ...
                <div>
                  <label className="block text-slate-200 font-semibold mb-1">Personal Notes (Optional)</label>
                  <textarea
                    rows={2}
                    value={agentNotes}
                ...
                <div className="pt-3 flex items-center justify-end gap-2">
                  <button
                    type="button"
                    onClick={() => setEmailModalOpen(false)}
                    className="px-4 py-2 rounded-xl bg-slate-800 text-slate-200 hover:bg-slate-800"
                  >
                    Cancel
                  </button>
                  <button
