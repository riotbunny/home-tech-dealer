...
export function CityDirectoryModal({ 
  ...
  return (
    ...
    >
      <div 
        className="relative w-full max-w-4xl max-h-[85vh] bg-slate-900 rounded-3xl shadow-2xl border border-slate-800 flex flex-col overflow-hidden text-white"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Modal Header */}
        <div className="p-5 sm:p-6 border-b border-slate-800/50 flex items-center justify-between bg-slate-950/50/80">
          ...
          <button
            onClick={onClose}
            className="w-9 h-9 rounded-xl bg-slate-900 border border-slate-800 text-slate-500 hover:text-slate-100 hover:bg-slate-800 flex items-center justify-center transition-colors"
        ...
        <div className="p-4 sm:p-5 border-b border-slate-800/50 space-y-3 bg-slate-900">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-2xl bg-slate-950/50 border border-slate-800 text-sm font-semibold text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:bg-slate-900 transition-all"
            />
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-6 divide-y divide-slate-100">
          {filteredStates.length === 0 ? (
            <div className="text-center py-12 text-slate-500">
              ...
              <p className="font-bold text-sm text-slate-200">No matching US cities found</p>
              <p className="text-xs text-slate-500 mt-1">Try typing another city name or select a state above.</p>
            </div>
          ) : (
              ...
              <div key={state.code} className="pt-5 first:pt-0">
                <div className="flex items-center justify-between gap-2 mb-3">
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-xs font-extrabold px-2 py-0.5 rounded-md bg-indigo-900/30 text-indigo-700 border border-blue-200">
                      {state.code}
                    </span>
                    ...
                    return (
                      <a
                        ...
                        className={`group p-3 rounded-xl border text-left transition-all flex items-center justify-between gap-2 ${
                          isCurrent
                            ? 'bg-indigo-900/30/80 border-blue-300 ring-2 ring-blue-200'
                            : 'bg-slate-900 hover:bg-slate-950/50 border-slate-800/80 hover:border-blue-300 hover:shadow-xs'
                        }`}
                      >
                        <div className="min-w-0">
                          <div className="flex items-center gap-1.5">
                            <MapPin className={`w-3.5 h-3.5 shrink-0 ${isCurrent ? 'text-indigo-600' : 'text-slate-500 group-hover:text-indigo-600'}`} />
                            <span className={`font-bold text-xs truncate ${isCurrent ? 'text-blue-900 font-extrabold' : 'text-slate-100'}`}>
                              {city.city}, {city.state}
                            </span>
                          </div>
                          <div className="text-[10px] text-slate-500 mt-0.5 pl-5 font-mono">
        ...
        {/* Modal Footer */}
        <div className="p-4 bg-slate-950/50 border-t border-slate-800/50 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-slate-500">
          <div>
            100% Nationwide Coverage &bull; Top 4 Guaranteed Carriers in All 50 States
          </div>
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-300 font-bold text-slate-100 transition-colors"
          >
            Close Directory
          </button>
        </div>
