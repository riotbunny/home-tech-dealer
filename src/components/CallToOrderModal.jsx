...
export function CallToOrderModal({ 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xs animate-fade-in">
      <div className="relative w-full max-w-lg bg-slate-900 border border-slate-800 rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col my-auto">
        
        {/* Top Header */}
        <div className="bg-slate-950/50 px-4 sm:px-6 py-3.5 sm:py-4 border-b border-slate-800 flex items-center justify-between">
        ...
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-5 overflow-y-auto max-h-[85vh]">
          ...
          <div className="p-3.5 sm:p-4 rounded-2xl bg-slate-950/50 border border-slate-800 flex items-start justify-between gap-3">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[10px] bg-slate-900 border border-slate-800 text-slate-300 px-1.5 py-0.5 rounded font-medium">
          ...
          <div className="p-4 sm:p-5 rounded-2xl bg-emerald-900/30/60 border border-emerald-200 text-center space-y-3">
            ...
            <div className="p-2.5 rounded-xl bg-slate-900 border border-emerald-200 text-xs text-slate-200 flex items-center justify-between">
              <span className="font-medium">Mention Promo Code:</span>
              <span className="font-bold text-emerald-700 bg-emerald-900/30 px-2 py-0.5 rounded border border-emerald-200">
                PROMO-FREE-INSTALL
              </span>
            </div>
          </div>
          ...
          <div className="pt-2 border-t border-slate-800/50">
            {callbackRequested ? (
              <div className="p-4 rounded-2xl bg-emerald-900/30 border border-emerald-200 text-center space-y-2">
                <div className="w-10 h-10 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center mx-auto">
                  <CheckCircle2 className="w-6 h-6" />
                </div>
                <h5 className="text-sm font-bold text-white">Callback Scheduled!</h5>
                <p className="text-xs text-slate-300 max-w-xs mx-auto">
                  A specialist will call <strong className="text-white font-semibold">{customerPhone}</strong> {callTime.toLowerCase()} to finalize your {selectedPlan.providerName} order.
                </p>
              ...
              <form onSubmit={handleCallbackSubmit} className="space-y-3">
                <div className="flex items-center justify-between text-xs">
                  <span className="font-bold text-slate-100">Can't call right now? We'll call you:</span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 bg-slate-950/50 border border-slate-700 rounded-xl text-white placeholder-slate-400 text-base sm:text-xs focus:outline-none focus:border-blue-600"
                  />
                  <select
                    value={callTime}
                    onChange={(e) => setCallTime(e.target.value)}
                    className="w-full px-3.5 py-2.5 bg-slate-950/50 border border-slate-700 rounded-xl text-white text-base sm:text-xs focus:outline-none focus:border-blue-600"
                  >
                ...
                <button
                  type="submit"
                  className="w-full py-2.5 rounded-xl bg-slate-800 hover:bg-slate-800 text-slate-100 text-xs font-bold transition-all flex items-center justify-center gap-1.5"
                >
